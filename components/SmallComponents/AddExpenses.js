...
const AddExpenses = (props) => {
    ...
    React.useState(() => {
        if(props.UserInformation){
            firestore().collection('ExpenseList').orderBy('Date', 'desc').onSnapshot(docs => {
                let arr = [];
                let cost = 0;
                docs.forEach(doc => {
    ...
    const AddCost = () => {
        if (Cost && Type) {
            setLoading(true);
            firestore().collection('ExpenseList').add({
                ...
                setLoading(false);
                Alert.alert('Success!', 'Expense Added Successful.');
                setCost('');
                setType('');
            })
        } else {
    ...
    return (
        <View style={{
            ...
            <TextInput theme={{
                colors: { primary: '#D35400' }
            }} onChangeText={(val) => setCost(val)} mode='flat' value={Cost}
                label="Cost" placeholder='Enter Cost..' style={styles.input} keyboardType='numeric' />
            <TextInput theme={{
                colors: { primary: '#D35400' }
            }} onChangeText={(val) => setType(val)} mode='flat' label="Details"
